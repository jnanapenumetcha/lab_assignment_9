...
/**
 * Modified by Sindhu on 25/10/2016.
 */
...
app.get('/getPlace', function (req, res) {
    ...
    request('https://api.edamam.com/search?q=chocolate&app_id=c009d437&app_key=6b01ffc1fdadadcb3b037b136b55549b', function (error, response, body) {
        //Check for error
        if(error){
            return console.log('Error:', error);
        }
        ...
        for(var i=0;i<ven.length;i++)
        {
            result.recipe.push({'recipe': ven[i].recipe.label});
        }
